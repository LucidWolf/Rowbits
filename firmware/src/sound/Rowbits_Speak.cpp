...
#include "Rowbits_Speak.h"
void Rowbits_Speak::_tone (float noteFrequency, long noteDuration, int silentDuration){   
      if(silentDuration==0){silentDuration=1;}
      tone(PIN_Buzzer, noteFrequency, noteDuration);
      delay(noteDuration);       //milliseconds to microseconds
      delay(silentDuration);     
}
void Rowbits_Speak::bendTones (float initFrequency, float finalFrequency, float prop, long noteDuration, int silentDuration){
...
}void Rowbits_Speak::sing(int songName){
  switch(songName){
